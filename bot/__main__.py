...
def start(update, context):
    ...
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
This Bot can mirror Anything To Google Drive, Type /{BotCommands.HelpCommand} To Get commands
'''
        sendMarkup(start_string, context.bot, update, reply_markup)
    else:
        sendMarkup(
            '❖ Not Authorized User. \n❖ Join Us @mirror_247_chennel',
            context.bot,
            update,
            reply_markup,
        )
...
def main():
    ...
    if os.path.isfile(".restartmsg"):
        ...
        bot.edit_message_text("❖ Successfully Restarted./n❖ Join Us @mirror_247_chennel", chat_id, msg_id)
...
main()
idle()
.LogCommand, log, filters=CustomFilters.owner_filter | CustomFilters.sudo_user, run_async=True)
    dispatcher.add_handler(start_handler)
    dispatcher.add_handler(ping_handler)
    dispatcher.add_handler(restart_handler)
    dispatcher.add_handler(help_handler)
    ...
    dispatcher.add_handler(log_handler)
    updater.start_polling(drop_pending_updates=IGNORE_PENDING_REQUESTS)
    LOGGER.info("ZX[LEECH]×͜× 𝚂𝚃𝙰𝚁𝚃𝙴𝙳!")
    signal.signal(signal.SIGINT, fs_utils.exit_clean_up)
